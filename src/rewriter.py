...
class UnifiedQueryRewriter:
    ...
    async def rewrite_query(
        ...
        system_prompt = (
            ...
            "4. **严格独立拆分**：不同知识点、不同领域的概念必须拆分为独立的 query group，"
            "绝对不允许将不相关的概念合并到同一组。\n"
            "   - 多选题（A/B/C/D）：每个选项对应一个独立 query group。\n"
            "   - 多个不同领域的问题：每个问题对应一个独立 query group。\n"
            "   - 同一知识点的多个相关术语：可以放在同一 query group 中。\n"
            ...
            "\n"
            "# Examples\n"
            "用户输入: \"判断正误\\nA.端粒成分中含有端粒RNA，是端粒酶的抑制剂\\nB.解链酶结合于前导模板链上\\nC.鞘磷脂合成于高尔基体需要VB6和亚铁离子\\nD.高倍镜可以改善分辨率\"\n"
            '正确输出: {"rewritten_queries": ["端粒 端粒RNA 端粒酶 抑制剂", "DNA复制 解链酶 前导链 模板链 结合位置", "鞘磷脂 合成 高尔基体 维生素B6 亚铁离子", "光学显微镜 低倍镜 高倍镜 分辨率 数值孔径"]}\n'
            '错误输出: {"rewritten_queries": ["端粒RNA 解链酶 鞘磷脂 高倍镜 分辨率"]}\n'
            "\n"
            ...
            "\n"
            "用户输入: \"萜类物质合成的途径有哪些\"\n"
            '正确输出: {"rewritten_queries": ["萜类 生物合成途径 甲羟戊酸途径 MVA", "萜类 甲基赤藓糖醇磷酸途径 MEP 质体"]}\n'
        )
